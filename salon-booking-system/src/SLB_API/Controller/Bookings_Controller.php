...
class Bookings_Controller extends REST_Controller
{
    ...
    public function get_items( $request )
    {
        ...
        if ($request->get_param('start_date_time')) {

            if ( ! isset( $prepared_args['meta_query'] ) ) {
                $prepared_args['meta_query'] = array();
            }

	    $tmp	 = explode(" ", $request->get_param('start_date_time'));
	    $_start_date = $tmp[0];
	    $_start_time = isset($tmp[1]) ? $tmp[1] : null;
        ...
        if ($request->get_param('end_date_time')) {

            if ( ! isset( $prepared_args['meta_query'] ) ) {
                $prepared_args['meta_query'] = array();
            }

	    $tmp	= explode(" ", $request->get_param('end_date_time'));
	    $_end_date	= $tmp[0];
	    $_end_time	= isset($tmp[1]) ? $tmp[1] : null;
	...
	$date = current_time('Y-m-d');
	    ...
	    'post_status'   => SLN_Enum_BookingStatus::CONFIRMED,
	    'meta_query'    => array(
		array(
		    array(
			'key'     => '_sln_booking_date',
	...
	do_action('sln_api_bookings_create_item_post_before_valid', $bb, $request);
        ...
        if ( ! $bb->isValid() ) {
            throw new \Exception(__( 'Sorry, booking is not available.', 'salon-booking-system' ));
        }

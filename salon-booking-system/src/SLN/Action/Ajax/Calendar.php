...
class SLN_Action_Ajax_Calendar extends SLN_Action_Ajax_Abstract
{
    ...
    public function execute()
    {
        ...
        $this->from = new SLN_DateTime(date("c", sanitize_text_field(wp_unslash($_GET['from']) ) / 1000 - $offset));
        $this->to = new SLN_DateTime(date("c", sanitize_text_field(wp_unslash($_GET['to']) ) / 1000 - $offset));
        $this->buildBookings();
